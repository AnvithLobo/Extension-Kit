...
cmd_execute_donut.addArgBool("--async", "Use Async BOF (managed by upstream)");
cmd_execute_donut.addArgBool("--token", "Use impersonated token for execution");
...
cmd_execute_donut.setPreHook(function (id, cmdline, parsed_json, ...parsed_lines) {
    ...
    var useToken = parsed_json["--token"] ? 1 : 0;
    var async_flag = parsed_json["--async"] ? "-a " : "";
    ...
    var bof_params = ax.bof_pack("int,cstr,cstr,bytes,int", [ppid, spawn, pipeName, shellcode_b64, useToken]);
    var bof_path = ax.script_dir() + "_bin/execute-donut." + ax.arch(id) + ".o";
    var message = "Task: execute-donut " + ax.file_basename(exe_path);

    ax.execute_alias(id, cmdline, `execute bof ${async_flag}"${bof_path}" ${bof_params}`, message);
});
